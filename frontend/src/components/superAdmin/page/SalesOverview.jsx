...
const SalesOverview = () => {
  ...
  useEffect(() => {
    const fetchData = async () => {
      try {
        const [dashboardResponse, salesResponse, storesResponse] =
          await Promise.all([
    ...
    <div className="space-y-6">
      ...
      <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-5 gap-4">
        ...
        <Card>
          ...
          <CardContent>
            <div className="text-2xl font-bold">
              {formatNumber(dashboardData.today_orders)}
            </div>
      ...
      <Card>
        ...
        <CardContent>
          <div className="h-80 flex items-end justify-between gap-2 border-l border-b border-gray-200 pl-4 pb-4">
            {(chartType === "daily" ? salesData.daily : salesData.monthly).map(
              (item, index) => {
                const dataSet =
                  chartType === "daily" ? salesData.daily : salesData.monthly;
